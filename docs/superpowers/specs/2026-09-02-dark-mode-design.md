...
**해석된 hex → 의미 토큰 문자열**이 된다. 값은 셋 중 하나다:

| 값 | 뜻 | 테마 의존 |
|---|---|---|
| `""` | 기본색 | — |
| `"31"`·`"92"`·`"41"` … | SGR 코드 (표준 16색) | **예** — 팔레트에서 찾는다 |
| `"#RRGGBB"` | 리터럴 색 | 아니오 — 그대로 쓴다 |

리터럴이 필요한 이유: 24비트 트루컬러(`38;2;r;g;b`)와 xterm256 의 컬러큐브(index ≥ 16)는
팔레트로 환원되지 않는 절대색이다. 반면 xterm256 의 **index < 16 은 표준 16색**이므로
SGR 코드로 환산해 담는다 — 그래야 테마를 따라간다.

(초안에서는 `int` 코드 + `X256_BASE` 오프셋을 쓰려 했으나 트루컬러를 표현할 수 없어
문자열 토큰으로 바꿨다.)
...
- 하이라이터가 span 토큰을 `ansi.resolve()` 로 풀고 **QColor 를 토큰별로 캐시**한다.
...
- **core (플랫폼 무관)**
  - `ansi`: 테마 교체 후 `resolve("32")` 가 다크 값을 돌려준다 / 두 팔레트의 키 집합이
    같다 / 알 수 없는 테마는 light 폴백 / 리터럴(`#RRGGBB`)은 테마와 무관하게 그대로 /
    xterm256 index<16 은 코드로, ≥16 은 리터럴로 담긴다
