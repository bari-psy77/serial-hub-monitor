# 프로젝트 현황 (기준일: 2026-08-14)

## 개요

**Serial Hub (v1.4.0)** — VS Code Serial Monitor + Tera Term + MobaXterm를 한 창으로 대체하는 윈도우 데스크톱 시리얼 모니터(PySide6 + pyserial). 최대 3개 콘솔(User CLI / Matter log / Matter shell)을 동시에 띄우고, 필터드뷰·ANSI 색상·수동 시작 로깅·PSK/networkkey 마스킹·트리거(WDOG/HardFault)·프로파일·localhost 자동화 브리지를 제공한다. 1.4.0에서 **지난 로그 파일 뷰어**와 **내장 ConPTY 터미널**이 추가됐다. PyInstaller + Inno Setup으로 설치본을 만든다.

## 현재까지 진행 사항

- **테스트 실측(2026-08-14)**: `python selftest.py` → **220 passed**, `python selftest.py --gui` → **299 passed**, `python uitest.py` → **162 passed**. 합계 461건 전부 통과. `ruff check . --select E,F,W,B,ARG --line-length 110` 클린.
...
- **실기 검증(이 세션)**: 터미널·뷰어·도크 동작을 **실제 디스플레이에서 스크린샷으로 확인**했다. 전각(한글) 정렬, PSReadLine 토큰 색, 배경색 블록, 휠 스크롤백, 재시작 후 깨끗한 화면, 탭 묶기, 닫은 뒤 레이아웃 복구까지 눈으로 대조.
...
- **배포 경로가 없다**: 릴리스 워크플로가 없고 `dist/`는 gitignore라, README가 안내하는 `dist/SerialHub_Setup_1.4.0.exe`는 클론한 사람에게 존재하지 않는다.
...
3. **배포 방식 결정** — GitHub Releases + 릴리스 워크플로를 만들 것인지, 사내 공유 폴더로 직접 전달할 것인지.
4. **코드 서명 인증서 구매 여부** — 사지 않는다면 "SmartScreen 경고는 정상"이라는 안내를 배포 문서에 명시해야 한다.
...
2. **배포 경로 확립 (반나절 — 차단 요소 3·4)**: 태그 푸시 시 exe/설치본을 굽고 Releases에 올리는 워크플로(윈도우 러너 필요), README의 `dist/` 안내를 실제 다운로드 경로로 교체.
